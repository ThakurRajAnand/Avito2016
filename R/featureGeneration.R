...
imp <- xgb.importance(feature_names = names(trainx), model = clf)
kable(imp)


pred <- predict(clf,data.matrix(testx))
sub <- data.frame(id=testItem$id.x,probability=pred)
write.csv(sub,file="../LB/T_0023.csv",row.names=FALSE)
...
pred <- predict(clf,data.matrix(testx))
sub <- data.frame(id=testItem$id.x,probability=pred)
write.csv(sub,file="../LB/T_0025.csv",row.names=FALSE)
