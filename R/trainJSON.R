...
trainJSON <- tr 
saveRDS(trainJSON,file="trainJSON.RDS")

tr[, c('itemID_1', 'itemID_2') := NULL]

response <- tr[['isDuplicate']]
tr[, isDuplicate := NULL]
tt <- tr[, lapply(.SD, function(x) {as.numeric(as.factor(x))})]

#==== Test ===

split <- sample.split(response, 0.8)
dtrain <- xgb.DMatrix(data = data.matrix(subset(tt, split)), label = response[split])
dval <- xgb.DMatrix(data = data.matrix(subset(tt, !split)), label = response[!split])
